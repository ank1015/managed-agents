...
import {
  ContractException, PI_WRITE_OPERATION, PI_WRITE_RECEIVER, parseWriteInput, parseWriteSubmission,
...
import { PI_WRITE_MAX_FILE_BYTES } from "@managed-agents/contracts";
...
import { parseWriteContext } from "./context.ts";
...
export class WriteService {
  ...
  submit(value: unknown): Promise<ProviderSubmitResult> {
    const logger = new Logger("write-worker", this.env), started = Date.now();
    ...
    return withDeadline<ProviderSubmitResult>(async signal => {
      ...
      try {
        ...
        if (request.provider !== PI_WRITE_OPERATION.provider || request.type !== PI_WRITE_OPERATION.type || request.version !== PI_WRITE_OPERATION.version) {
          return { status: "rejected", error: { code: "UNSUPPORTED_PI_WRITE_OPERATION", message: "Expected tool-pi-write/write/v1." } };
        }
      ...
      const idempotencyKey = await writeIdentity(submissionId);
      const contentBytes = Buffer.byteLength(input.content, "utf8");
      if (contentBytes > PI_WRITE_MAX_FILE_BYTES) {
        // Stable local receipt: no gateway acceptance or side effect took place.
        return { status: "completed", jobId: `local:${idempotencyKey}`, outcome: completed(fileTooLarge(input)) };
      }
      const clientContext = { receiver: PI_WRITE_RECEIVER, ...parsed.destination, operationId, submissionId,
        machineId: input.machineId, cwd: input.cwd, path: input.path, contentBytes, contentSha256: await sha256(input.content) };
  ...
  acceptGatewayEvent(value: unknown): Promise<GatewayEventReceipt> {
    ...
    return withDeadline(async signal => {
      const event = parseExecutionGatewayEvent(value), context = parseWriteContext(event.clientContext);
      if (event.machineId !== context.machineId || event.idempotencyKey !== await writeIdentity(context.submissionId)) {
        throw new Error("Gateway event conflicts with write submission context.");
      }
      ...
      const reply = await namespace.get(namespace.idFromName(context.sessionId)).sessionRequest({
        action: "acceptCompletion", value: { provider: "tool-pi-write", operationId: context.operationId,
      ...
      if (!(reply as { value: { duplicate: boolean } }).value.duplicate) {
        const logger = new Logger("write-worker", this.env);
        const fields = {
