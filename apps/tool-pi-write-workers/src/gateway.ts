...
import type { WriteInput, JsonValue, OperationOutcome } from "@managed-agents/contracts";
...
export interface Job { id: string; status: JobStatus; idempotencyKey: string; machineId: string; runtimeGenerationId: string | null; clientContext: WriteContext }
...
function job(value: unknown): Job {
  ...
  return { clientContext: parseWriteContext(r.clientContext), id: parseUuid(r.id), status: r.status as JobStatus,
...
export class Gateway {
  ...
  async submit(input: WriteInput, idempotencyKey: string, clientContext: WriteContext): Promise<{ id: string; status: JobStatus }> {
    const result = object(await this.request("/v1/jobs", {
      machineId: input.machineId, idempotencyKey, clientContext,
      request: { operation: "filesystem.write_file", params: { path: input.path, cwd: input.cwd, mutation_id: idempotencyKey,
        data_base64: Buffer.from(input.content, "utf8").toString("base64"), create_parent_directories: true, mode: "overwrite" } },
    }));
  ...
  async outcome(expected: ExpectedJob): Promise<OperationOutcome> {
    // Only terminal submission replay reads detail. Callbacks contain the signed receipt and expected content digest.
    // Job detail also includes the retained base64 request (up to the gateway frame limit).
