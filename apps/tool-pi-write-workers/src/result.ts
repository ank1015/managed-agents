...
export function writeError(context: Pick<WriteContext, "machineId" | "path">, code: string, message: string, jobId?: string): WriteResult {
...
export async function formatReceipt(value: unknown, context: WriteContext, jobId: string): Promise<WriteResult> {
  ...
  return { content: [{ type: "text", text: `Successfully wrote to ${context.path}` }], isError: false,
    details: { gatewayJobId: jobId, machineId: context.machineId, path: context.path,
