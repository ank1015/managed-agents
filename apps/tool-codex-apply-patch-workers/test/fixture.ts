...
import { CODEX_APPLY_PATCH_OPERATION, parseApplyPatchInput, parseJsonValue, parseProviderSubmitReply, parseSessionCommand, parseGatewayEventReply } from "@managed-agents/contracts";
import type { EventBody, JsonValue, ApplyPatchWorkerBinding, GatewayEventReceiverBinding, SessionReply } from "@managed-agents/contracts";
...
interface TestEnv extends Env { APPLY_PATCH: ApplyPatchWorkerBinding; APPLY_PATCH_EVENTS: GatewayEventReceiverBinding; SESSIONS: DurableObjectNamespace<TestSession> }
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "/event") return Response.json(parseGatewayEventReply(await env.APPLY_PATCH_EVENTS.acceptGatewayEvent(body)));
