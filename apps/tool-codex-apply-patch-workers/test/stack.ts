...
import type { ApplyPatchContext } from "../src/context.ts";
...
export const input = { machineId, cwd: "/workspace/project", patch: "*** Begin Patch\n*** Update File: file.txt\n@@\n-old\n+new\n*** End Patch" };
export interface FakeJob {
  id: string; machineId: string; runtimeGenerationId: string | null; idempotencyKey: string; status: string;
  clientContext: ApplyPatchContext;
  request: { operation: string; params: { cwd: string; mutation_id: string; patch: { format: string; text: string } } };
  response: JsonValue; error: JsonValue;
}
export function patchReceipt(job: FakeJob) {
  const p = job.request.params;
  return { mutation_id: p.mutation_id, status: "applied", changes_exact: true,
    changes: [{ kind: "update", path: `${p.cwd}/file.txt`,
...
export class FakeGateway {
  ...
  async fetch(request: MFRequest): Promise<MFResponse> {
    ...
    // Job detail includes the retained request, including patch text.
    return MFResponse.json(this.detailValue ? this.detailValue(job) : { ...job, requestStatus: "retained" });
  }
  finish(job: FakeJob, value: JsonValue = patchReceipt(job)) {
    job.status = (value as { status?: string }).status === "applied" ? "succeeded" : "failed"; job.error = null;
...
export async function startStack(gateway = new FakeGateway(), options: { persistPath?: string } = {}) {
  ...
  const app = new Miniflare({ log: new Log(LogLevel.ERROR), workers: [
    { ...common, name: "apply-patch", script: worker, durableObjects: { SESSIONS: { className: "TestSession", scriptName: "host" } } },
    { ...common, name: "host", script: fixture, serviceBindings: { APPLY_PATCH: { name: "apply-patch", entrypoint: "CodexApplyPatch" }, APPLY_PATCH_EVENTS: { name: "apply-patch", entrypoint: "CodexApplyPatchCallbacks" } },
    ...
    { modules: true, compatibilityDate: "2026-07-30", name: "callbacks", script: callbackWorker,
      bindings: { EXECUTION_GATEWAY_WEBHOOK_SECRET: secret, CALLBACK_ROUTES: JSON.stringify({ "tool-codex-apply-patch-v1": "APPLY_PATCH_EVENTS" }) },
      serviceBindings: { APPLY_PATCH_EVENTS: { name: "apply-patch", entrypoint: "CodexApplyPatchCallbacks" } },
  ...
  try {
    const host = await app.getWorker("host"), patchWorker = await app.getWorker("apply-patch"), callbacks = await app.getWorker("callbacks");
    return { app, gateway, patchWorker, callbacks,
