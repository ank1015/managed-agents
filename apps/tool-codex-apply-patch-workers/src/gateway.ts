...
import type { ApplyPatchInput, JsonValue, OperationOutcome } from "@managed-agents/contracts";
...
export interface Job { id: string; status: JobStatus; idempotencyKey: string; machineId: string; runtimeGenerationId: string | null; clientContext: ApplyPatchContext }
...
export function patchParams(input: ApplyPatchInput, mutationId: string) {
  return { mutation_id: mutationId, cwd: input.cwd,
    patch: { format: "codex", text: input.patch } };
}
...
function job(value: unknown): Job {
  ...
  return { clientContext: parseApplyPatchContext(r.clientContext), id: parseUuid(r.id), status: r.status as JobStatus,
...
export class Gateway {
  ...
  async submit(input: ApplyPatchInput, idempotencyKey: string, clientContext: ApplyPatchContext): Promise<{ id: string; status: JobStatus }> {
