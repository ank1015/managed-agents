...
import { formatReceipt, object, applyPatchError } from "./result.ts";
...
export async function terminalOutcome(job: Pick<Job, "id" | "status" | "machineId" | "runtimeGenerationId" | "idempotencyKey">,
  responseValue: unknown, errorValue: unknown, context: ApplyPatchContext, signal: AbortSignal): Promise<OperationOutcome> {
  if (!["succeeded", "failed", "unknown"].includes(job.status)) throw new Error("Expected a terminal execution job.");
  if (job.status === "unknown") return failure(job.id, "APPLY_PATCH_OUTCOME_UNKNOWN", "The patch outcome is unknown and files may have changed. Inspect the affected files before deciding how to proceed. This operation will not be automatically replaced with a new patch.");
  if (errorValue !== null) {
  ...
  if (response.status === "error" && job.status === "failed") {
    ...
    if (["invalid_argument", "resource_limit"].includes(error.code)) {
      return completed(applyPatchError(context, error.code, error.message.slice(0, 2000), job.id));
    }
    return failure(job.id, error.code.slice(0, 200), error.message.slice(0, 1000));
  }
