...
import {
  ContractException, CODEX_APPLY_PATCH_OPERATION, CODEX_APPLY_PATCH_RECEIVER, parseApplyPatchInput, parseApplyPatchSubmission,
...
import { CODEX_APPLY_PATCH_MAX_PARAMS_BYTES, CODEX_APPLY_PATCH_MAX_TEXT_BYTES } from "@managed-agents/contracts";
...
import { parseApplyPatchContext } from "./context.ts";
...
export class ApplyPatchService {
  ...
  submit(value: unknown): Promise<ProviderSubmitResult> {
    const logger = new Logger("apply-patch-worker", this.env), started = Date.now();
    ...
    return withDeadline<ProviderSubmitResult>(async signal => {
      ...
      try {
        ...
        if (request.provider !== CODEX_APPLY_PATCH_OPERATION.provider || request.type !== CODEX_APPLY_PATCH_OPERATION.type || request.version !== CODEX_APPLY_PATCH_OPERATION.version) {
          return { status: "rejected", error: { code: "UNSUPPORTED_CODEX_APPLY_PATCH_OPERATION", message: "Expected tool-codex-apply-patch/apply_patch/v1." } };
        }
      ...
      const idempotencyKey = await applyPatchIdentity(submissionId);
      if (Buffer.byteLength(input.patch, "utf8") > CODEX_APPLY_PATCH_MAX_TEXT_BYTES
        || Buffer.byteLength(JSON.stringify(patchParams(input, idempotencyKey)), "utf8") > CODEX_APPLY_PATCH_MAX_PARAMS_BYTES) {
      ...
      const clientContext = { receiver: CODEX_APPLY_PATCH_RECEIVER, ...parsed.destination, operationId, submissionId,
        machineId: input.machineId, cwd: input.cwd, patchSha256: await sha256(input.patch) };
  ...
  acceptGatewayEvent(value: unknown): Promise<GatewayEventReceipt> {
    ...
    return withDeadline(async signal => {
      const event = parseExecutionGatewayEvent(value), context = parseApplyPatchContext(event.clientContext);
      if (event.machineId !== context.machineId || event.idempotencyKey !== await applyPatchIdentity(context.submissionId)) {
        throw new Error("Gateway event conflicts with patch submission context.");
      }
      ...
      const reply = await namespace.get(namespace.idFromName(context.sessionId)).sessionRequest({
        action: "acceptCompletion", value: { provider: "tool-codex-apply-patch", operationId: context.operationId,
      ...
      if (!(reply as { value: { duplicate: boolean } }).value.duplicate) {
        const logger = new Logger("apply-patch-worker", this.env);
        const fields = {
