...
import { PI_EDIT_OPERATION, parseEditInput, parseJsonValue, parseProviderSubmitReply, parseSessionCommand, parseGatewayEventReply } from "@managed-agents/contracts";
import type { EventBody, JsonValue, EditWorkerBinding, GatewayEventReceiverBinding, SessionReply } from "@managed-agents/contracts";
...
interface TestEnv extends Env { EDIT: EditWorkerBinding; EDIT_EVENTS: GatewayEventReceiverBinding; SESSIONS: DurableObjectNamespace<TestSession> }
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "/event") return Response.json(parseGatewayEventReply(await env.EDIT_EVENTS.acceptGatewayEvent(body)));
