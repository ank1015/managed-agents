...
import type { EditContext } from "../src/context.ts";
...
export interface FakeJob {
  ...
  clientContext: EditContext;
  request: { operation: string; params: { cwd: string; mutation_id: string; patch: { format: string; files: { path: string; edits: { oldText: string; newText: string }[] }[] } } };
  response: JsonValue; error: JsonValue;
}
export function editReceipt(job: FakeJob) {
  const p = job.request.params, file = p.patch.files[0]!;
...
export class FakeGateway {
  ...
  finish(job: FakeJob, value: JsonValue = editReceipt(job)) {
...
export async function startStack(gateway = new FakeGateway(), options: { persistPath?: string } = {}) {
  ...
  const app = new Miniflare({ log: new Log(LogLevel.ERROR), workers: [
    ...
    { ...common, name: "host", script: fixture, serviceBindings: { EDIT: { name: "edit", entrypoint: "PiEdit" }, EDIT_EVENTS: { name: "edit", entrypoint: "PiEditCallbacks" } },
    ...
    { modules: true, compatibilityDate: "2026-07-30", name: "callbacks", script: callbackWorker,
      bindings: { EXECUTION_GATEWAY_WEBHOOK_SECRET: secret, CALLBACK_ROUTES: JSON.stringify({ "tool-pi-edit-v1": "EDIT_EVENTS" }) },
      serviceBindings: { EDIT_EVENTS: { name: "edit", entrypoint: "PiEditCallbacks" } },
  ...
  try {
    const host = await app.getWorker("host"), edit = await app.getWorker("edit"), callbacks = await app.getWorker("callbacks");
    return { app, gateway, edit, callbacks,
