...
function signed(value: Record<string, unknown>) {
  ...
  return new Request("https://test/webhook", { method: "POST", body, headers: {
    "Content-Type": "application/json", "X-Execution-Gateway-Timestamp": timestamp,
    "X-Execution-Gateway-Event-Id": String(value.eventId),
    "X-Execution-Gateway-Signature": "v1=" + createHmac("sha256", secret).update(timestamp + "." + value.eventId + "." + body).digest("hex"),
  } });
}
test("authenticated malformed callbacks and failed delivery log once without inline payloads", async t => {
  ...
  const env = { EXECUTION_GATEWAY_WEBHOOK_SECRET: secret, LOG_SUCCESS_SAMPLE_RATE: "0", CALLBACK_ROUTES: JSON.stringify({ "tool-pi-bash-v1": "BASH" }),
BASH: { acceptGatewayEvent: async () => { throw new Error("private upstream response"); } }, };
  ...
  logs.length = 0;
  const evt = { schemaVersion: 3, eventId: randomUUID(), jobId: randomUUID(), machineId: randomUUID(), runtimeGenerationId: randomUUID(),
idempotencyKey: "test-key", type: "job.succeeded", completedAt: new Date().toISOString(),
clientContext: { receiver: "tool-pi-bash-v1", sessionId, operationId: "replay-v1:1:" + "a".repeat(64) },
response: { result: "private tool output" }, error: null };
