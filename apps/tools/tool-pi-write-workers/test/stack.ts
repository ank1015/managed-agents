...
import type { OperationOutcome, WriteExecutionContext } from "@managed-agents/contracts";
export const auth = "write-test-signing-secret-at-least-32-bytes";
...
export async function execution(sessionId: string, routeKey = "test-v1"): Promise<WriteExecutionContext> {
...
export function writeReceipt(job: FakeJob) {
  const p = job.body.operation.params as any, bytes = Buffer.from(p.content.data, "base64");
  return { path: p.path.startsWith("/") ? p.path : `${p.cwd}/${p.path}`, mutation_id: job.id,
...
export class FakeGateway {
  ...
  finish(job: FakeJob, value = writeReceipt(job)) { job.outcome = { status: "ok", result: value }; }
...
export async function startStack(gateway = new FakeGateway(), options: { persistPath?: string } = {}) {
  ...
  const app = new Miniflare({ log: new Log(LogLevel.ERROR), workers: [
    ...
    { ...common, name: "host", script: fixture, serviceBindings: { WRITE: { name: "write", entrypoint: "PiWrite" }, WRITE_EVENTS: { name: "write", entrypoint: "PiWriteCallbacks" } }, durableObjects: { SESSIONS: { className: "TestSession", useSQLite: true } } },
  ], ...(options.persistPath ? { durableObjectsPersist: `${options.persistPath}/objects` } : {}) });
  const host = await app.getWorker("host"), write = await app.getWorker("write");
  return { app, gateway, write, async call<T = Snapshot>(path: string, body: unknown = {}): Promise<T> {
