...
export class TestSession extends DurableObject<TestEnv> {
  ...
  async sessionRequest(value: unknown): Promise<SessionReply<unknown>> {
    ...
    const input = parseWriteInput(this.driver.getSession().config);
...
async function execution(sessionId: string) {
  ...
  return {...stored, gatewayUrl: "https://gateway.test", token: await issueMachineSecret("write-test-signing-secret-at-least-32-bytes", "00000000-0000-4000-8000-000000000001", "execution", 1)};
}
