...
All use the [tool-facing gateway contract](GATEWAY-CONTRACT.md). Our
[machine-only gateway](../../execution/apps/execution-gateway/README.md) is one implementation;
apps may supply their own compliant gateway and daemon.
Harness hosts supply `{ destination, execution: { gatewayUrl, token, runtimeGeneration }, submission }`
