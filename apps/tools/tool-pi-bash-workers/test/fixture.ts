...
import { PI_BASH_OPERATION, parseBashInput, parseJsonValue, parseProviderSubmitReply, parseSessionCommand } from "@managed-agents/contracts";
import type { EventBody, JsonValue, BashWorkerBinding, SessionReply } from "@managed-agents/contracts";
...
interface TestEnv extends Env { BASH: BashWorkerBinding; BASH_EVENTS: { acceptExecutionResult(value: unknown): Promise<unknown> }; SESSIONS: DurableObjectNamespace<TestSession> }
const harness: HarnessDefinition<JsonValue, EventBody> = {
  ...
  handle(input, ctx) {
    return { changes: input.event.type === "start" ? null : { eventId: input.eventId, event: input.event },
      operations: input.event.type === "start" ? [{ key: "request", ...PI_BASH_OPERATION, input: ctx.config as JsonValue }] : [] };
  },
  apply(changes, ctx) {
    if (changes) {
...
export class TestSession extends DurableObject<TestEnv> {
  ...
  constructor(ctx: DurableObjectState, env: TestEnv) {
    ...
    this.driver = new SessionDriver(ctx.storage, harness, {
      providers: { "tool-pi-bash": {
        submit: async submission => parseProviderSubmitReply(await env.BASH.submit({
  ...
  async sessionRequest(value: unknown): Promise<SessionReply<unknown>> {
    ...
    const input = parseBashInput(this.driver.getSession().config);
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "/submit") return Response.json(parseProviderSubmitReply(await env.BASH.submit(body)));
      if (path === "/event") return Response.json(await env.BASH_EVENTS.acceptExecutionResult(body));
      if (path === "/callback-submit") return Response.json(await (env.BASH_EVENTS as unknown as BashWorkerBinding).submit(body));
...
async function execution(sessionId: string) {
  ...
  return {...stored, gatewayUrl: "https://gateway.test", token: await issueMachineSecret("bash-test-signing-secret-at-least-32-bytes", "00000000-0000-4000-8000-000000000001", "execution", 1)};
}
