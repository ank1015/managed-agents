...
import { FakeGateway, startStack, input, execution, event, until, auth, machineId, generationId, editReceipt } from "./stack.ts";
import type { Stack } from "./stack.ts";

async function submission() {
  return { execution: await execution("direct"), destination: { routeKey: "test-v1", sessionId: "direct" },
    submission: { operationId: "op", submissionId: "op", request: { provider: "tool-pi-edit", type: "edit", version: "v1", input: structuredClone(input) as EditInput } } };
}
async function start(s: Stack, value: EditInput = input) {
...
async function result(s: Stack, sessionId: string): Promise<EditResult> {
...
test("new operation, private callback and durable deduplicated completion", async t => {
  ...
  assert.deepEqual(job.body.operation, { operation: "filesystem.patch", params: { cwd: input.cwd, patch: { format: "text_replacements", files: [{ path: input.path, edits: input.edits }] } } });
  ...
  assert.equal((await s.edit.fetch("https://tool/webhooks/execution-gateway", { method: "POST" })).status, 404);
...
test("malformed envelopes and wrong machines cannot dispatch; gateway fences stale runtimes", async t => {
  ...
  for (const [status, code, retryable, uncertain] of [[409,"MACHINE_OFFLINE",true,false],[401,"INVALID_TOKEN",false,false],[409,"RUNTIME_CHANGED",false,false],[409,"REQUEST_CONFLICT",false,true],[429,"EditACITY",true,false],[400,"SUBMISSION_UNCERTAIN",false,true]] as const) {
...
test("native errors and uncertain outcomes remain distinct", async t => {
  ...
  assert.equal(failed.status, "failed"); if (failed.status === "failed") assert.equal(failed.error.code, "EDIT_OUTCOME_UNKNOWN");
});
test("completion admission survives Session DO restart", async () => {
  const dir = await mkdtemp(join(tmpdir(), "edit-session-")), gateway = new FakeGateway();
...
test("patch receipts preserve diff, rejection and partial-change details; bad mutation IDs cannot complete", async t => {
  const s = await startStack(); t.after(() => s.app.dispose());
  for (const status of ["applied", "rejected", "partial"] as const) {
    const { sessionId, job } = await start(s), receipt = editReceipt(job);
    s.gateway.finish(job, { ...receipt, status, changes_exact: status !== "partial", changes: status === "rejected" ? [] : receipt.changes,
      error: status === "applied" ? null : { code: "io", message: "failed", section: 0, edit: 0 } });
    await s.call("/event", await event(job)); const r = await result(s, sessionId);
    assert.equal(r.isError, status !== "applied"); assert.equal(r.details.status, status); assert.equal(r.details.diff, receipt.diff);
    if (status === "partial") assert.match(r.content[0]!.text, /file may have changed/);
  }
  const o = await start(s); s.gateway.finish(o.job, { ...editReceipt(o.job), mutation_id: "wrong" });
  await assert.rejects(s.call("/event", await event(o.job)));
  assert.equal((await s.call("/snapshot", { sessionId: o.sessionId })).admittedCompletions, 0);
});
test("oversized patch completes locally and never dispatches", async t => {
  const s = await startStack(); t.after(() => s.app.dispose()); const value = await submission();
  value.submission.request.input.edits = [{ oldText: "old", newText: "x".repeat(4 * 1024 * 1024) }];
  const reply = await s.call<ProviderSubmitResult>("/submit", value);
  assert.equal(reply.status, "completed");
  if (reply.status === "completed" && reply.outcome.status === "succeeded") assert.equal((reply.outcome.result as EditResult).details.error?.code, "EDIT_REQUEST_TOO_LARGE");
...
test("each submission forwards its own machine secret only in the authorization header", async t => {
  ...
  for (const value of [first, second]) {
    ...
    assert.equal((sent.body as any).callback.receiver, "tool-pi-edit-v1");
