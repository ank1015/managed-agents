...
import { PI_EDIT_OPERATION, parseEditInput, parseJsonValue, parseProviderSubmitReply, parseSessionCommand } from "@managed-agents/contracts";
import type { EventBody, JsonValue, EditWorkerBinding, SessionReply } from "@managed-agents/contracts";
...
interface TestEnv extends Env { EDIT: EditWorkerBinding; EDIT_EVENTS: { acceptExecutionResult(value: unknown): Promise<unknown> }; SESSIONS: DurableObjectNamespace<TestSession> }
const harness: HarnessDefinition<JsonValue, EventBody> = {
  identity: { id: "edit-test", version: "v1" }, operations: [PI_EDIT_OPERATION],
  schema: ["CREATE TABLE test_results (event_id TEXT PRIMARY KEY, payload TEXT NOT NULL)"],
  parseConfig(value) { return parseJsonValue(parseEditInput(value)); },
  parseInput: event => event,
  initialize() {},
  handle(input, ctx) {
    return { changes: input.event.type === "start" ? null : { eventId: input.eventId, event: input.event },
      operations: input.event.type === "start" ? [{ key: "request", ...PI_EDIT_OPERATION, input: ctx.config as JsonValue }] : [] };
  },
  apply(changes, ctx) {
    if (changes) {
...
export class TestSession extends DurableObject<TestEnv> {
  ...
  constructor(ctx: DurableObjectState, env: TestEnv) {
    ...
    this.driver = new SessionDriver(ctx.storage, harness, {
      providers: { "tool-pi-edit": {
        submit: async submission => parseProviderSubmitReply(await env.EDIT.submit({
  ...
  async sessionRequest(value: unknown): Promise<SessionReply<unknown>> {
    ...
    const input = parseEditInput(this.driver.getSession().config);
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "/submit") return Response.json(parseProviderSubmitReply(await env.EDIT.submit(body)));
      if (path === "/event") return Response.json(await env.EDIT_EVENTS.acceptExecutionResult(body));
      if (path === "/callback-submit") return Response.json(await (env.EDIT_EVENTS as unknown as EditWorkerBinding).submit(body));
...
async function execution(sessionId: string) {
  ...
  return {...stored, gatewayUrl: "https://gateway.test", token: await issueMachineSecret("edit-test-signing-secret-at-least-32-bytes", "00000000-0000-4000-8000-000000000001", "execution", 1)};
}
