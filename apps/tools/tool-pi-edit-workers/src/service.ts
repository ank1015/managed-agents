...
import { ContractException, PI_EDIT_OPERATION, PI_EDIT_MAX_PARAMS_BYTES, parseEditInput, parseEditSubmission } from "@managed-agents/contracts";
...
import { editIdentity, parseEditCompletion, assertSecretMatches } from "./context.ts";
...
export class EditService {
  ...
  submit(value: unknown): Promise<ProviderSubmitResult> {
    return withDeadline(async signal => {
      ...
      try {
        ...
        if (request.provider !== PI_EDIT_OPERATION.provider || request.type !== PI_EDIT_OPERATION.type || request.version !== PI_EDIT_OPERATION.version) throw new ContractException("INVALID_REQUEST", "Expected tool-pi-edit/edit/v1.");
      ...
      try { await new Gateway(this.env, signal).submit(input, parsed, requestId, context); }
        ...
        // Lost/malformed acceptance, credential replacement, runtime fences and conflicts never authorize a fresh edit.
        throw error;
      }
      new Logger("edit-worker", this.env).success("operation_submitted", { sessionId: parsed.destination.sessionId, operationId, stage: "submit", outcome: "accepted" });
  ...
  acceptExecutionResult(value: unknown): Promise<CompletionReply> {
    return withDeadline(async signal => {
      ...
      signal.throwIfAborted();
      if (!validReceipt(reply, context.operationId)) throw Error("Session did not confirm durable edit completion admission.");
