...
import type { EditInput, EditSubmission } from "@managed-agents/contracts";
import { GatewayError, MAX_HTTP_BYTES, submission, hashJson, jsonValue, object } from "@managed-agents/execution-gateway-protocol";
import { PI_EDIT_RECEIVER } from "@managed-agents/contracts";
...
export class Gateway {
  ...
  async submit(input: EditInput, parsed: EditSubmission, requestId: string, context: EditContext): Promise<void> {
    const body = submission({ requestId, runtimeGeneration: parsed.execution.runtimeGeneration, operation: { operation: "filesystem.patch", params: patchParams(input) }, callback: { receiver: PI_EDIT_RECEIVER, context: jsonValue(context) } });
    const encoded = JSON.stringify(body);
    if (new TextEncoder().encode(encoded).byteLength > MAX_HTTP_BYTES) throw new GatewayError(413, "EDIT_REQUEST_TOO_LARGE", "Edit request exceeds the gateway transfer limit.");
...
export function patchParams(input: EditInput) {
  return { cwd: input.cwd, patch: { format: "text_replacements", files: [{ path: input.path, edits: input.edits }] } };
}
