...
import { FakeGateway, startStack, input, execution, event, until, auth, machineId, generationId, fileResult, FakeImages, png } from "./stack.ts";
import type { Stack } from "./stack.ts";

async function submission() {
  return { execution: await execution("direct"), destination: { routeKey: "test-v1", sessionId: "direct" },
    submission: { operationId: "op", submissionId: "op", request: { provider: "tool-pi-read", type: "read", version: "v1", input: structuredClone(input) as ReadInput } } };
}
async function start(s: Stack, value: ReadInput = input) {
...
async function result(s: Stack, sessionId: string): Promise<ReadResult> {
...
test("new operation, private callback and durable deduplicated completion", async t => {
  ...
  assert.deepEqual(job.body.operation, { operation: "filesystem.read", params: { path: input.path, cwd: input.cwd, mode: "bytes" } });
  ...
  assert.equal((await s.read.fetch("https://tool/webhooks/execution-gateway", { method: "POST" })).status, 404);
...
test("stable identity survives lost acceptance and execution secret rotation; changed input conflicts", async t => {
  ...
  s.gateway.acceptance = undefined; value.submission.request.input.path = "changed";
...
test("malformed envelopes and wrong machines cannot dispatch; gateway fences stale runtimes", async t => {
  ...
  for (const [status, code, retryable, uncertain] of [[409,"MACHINE_OFFLINE",true,false],[401,"INVALID_TOKEN",false,false],[409,"RUNTIME_CHANGED",false,false],[409,"REQUEST_CONFLICT",false,true],[429,"ReadACITY",true,false],[400,"SUBMISSION_UNCERTAIN",false,true]] as const) {
...
test("native errors and uncertain outcomes remain distinct", async t => {
  ...
  for (const code of ["not_found", "io", "invalid_argument", "resource_limit"]) {
    const { sessionId, job } = await start(s); s.gateway.fileError(job, code, "Expected filesystem error", ["io", "resource_limit"].includes(code));
    await s.call("/event", await event(job)); assert.equal((await result(s, sessionId)).isError, true);
  }
  ...
  assert.equal(failed.status, "failed"); if (failed.status === "failed") assert.equal(failed.error.code, "READ_OUTCOME_UNKNOWN");
});
test("completion admission survives Session DO restart", async () => {
  const dir = await mkdtemp(join(tmpdir(), "read-session-")), gateway = new FakeGateway();
  let s = await startStack(gateway, new FakeImages(), { persistPath: dir });
  try {
    ...
    s = await startStack(gateway, new FakeImages(), { persistPath: dir }); await s.call("/event", completion);
    assert.equal((await s.call("/snapshot", { sessionId })).results.length, 1);
  } finally { await s.app.dispose(); await rm(dir, { recursive: true, force: true }); }
});
test("read paging, native byte integrity and file size limit", async t => {
  const s = await startStack(); t.after(() => s.app.dispose());
  const paged = await start(s, { ...input, offset: 2, limit: 1 });
  s.gateway.finish(paged.job, fileResult("a\nb\nc\n")); await s.call("/event", await event(paged.job));
  assert.deepEqual((await result(s, paged.sessionId)).content, [{ type: "text", text: "b\n\n[2 more lines in file. Use offset=3 to continue.]" }]);
  const other = await start(s), bytes = fileResult();
  s.gateway.finish(other.job, { ...bytes, file: { ...bytes.file, sha256: "0".repeat(64) } });
  await assert.rejects(s.call("/event", await event(other.job)));
  assert.equal((await s.call("/snapshot", { sessionId: other.sessionId })).admittedCompletions, 0);
  s.gateway.finish(other.job, { ...bytes, file: { ...bytes.file, size_bytes: 5242881 } });
  await s.call("/event", await event(other.job));
  assert.equal((await result(s, other.sessionId)).details.error?.code, "READ_FILE_TOO_LARGE");
});
test("image upload recovers lost responses and durable admission without duplicate stored images", async t => {
  const s = await startStack(); t.after(() => s.app.dispose());
  const { sessionId, job } = await start(s); s.gateway.finish(job, fileResult(png)); s.images.loseUpload = 1;
  await s.call("/faults", { sessionId, lose: 1 });
  await assert.rejects(s.call("/event", await event(job)));
  await s.call("/event", await event(job));
  const r = await result(s, sessionId);
  assert.ok(r.content.some(p => p.type === "image" && p.url.endsWith("/piread")));
  assert.deepEqual(s.images.uploadBytes, png); assert.equal(s.images.posts, 1);
  assert.equal((await s.call("/snapshot", { sessionId })).results.length, 1);
});
test("image API outage remains retryable; definitive rejection is a tool error", async t => {
  const s = await startStack(); t.after(() => s.app.dispose());
  const { sessionId, job } = await start(s); s.gateway.finish(job, fileResult(png)); s.images.getFailure = 503;
  await assert.rejects(s.call("/event", await event(job)));
  assert.equal((await s.call("/snapshot", { sessionId })).admittedCompletions, 0);
  s.images.getFailure = undefined; s.images.uploadFailure = 415;
  await s.call("/event", await event(job));
  assert.equal((await result(s, sessionId)).details.error?.code, "READ_IMAGE_REJECTED");
});
...
test("each submission forwards its own machine secret only in the authorization header", async t => {
  ...
  for (const value of [first, second]) {
    ...
    assert.equal((sent.body as any).callback.receiver, "tool-pi-read-v1");
