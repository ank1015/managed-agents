...
export const auth = "apply-patch-test-signing-secret-at-least-32-bytes";
...
export function patchReceipt(job: FakeJob) {
  const p = job.body.operation.params as { cwd: string };
...
export class FakeGateway {
  ...
  finish(job: FakeJob, value: unknown = patchReceipt(job)) { job.outcome = { status: "ok", result: jsonValue(value) }; }
...
export async function startStack(gateway = new FakeGateway(), options: { persistPath?: string } = {}) {
  ...
  const app = new Miniflare({ log: new Log(LogLevel.ERROR), workers: [
    ...
    { ...common, name: "host", script: fixture, serviceBindings: { APPLY_PATCH: { name: "apply-patch", entrypoint: "CodexApplyPatch" }, APPLY_PATCH_EVENTS: { name: "apply-patch", entrypoint: "CodexApplyPatchCallbacks" } }, durableObjects: { SESSIONS: { className: "TestSession", useSQLite: true } } },
  ], ...(options.persistPath ? { durableObjectsPersist: `${options.persistPath}/objects` } : {}) });
  const host = await app.getWorker("host"), patchWorker = await app.getWorker("apply-patch");
  return { app, gateway, patchWorker, async call<T = Snapshot>(path: string, body: unknown = {}): Promise<T> {
