...
export class TestSession extends DurableObject<TestEnv> {
  ...
  async sessionRequest(value: unknown): Promise<SessionReply<unknown>> {
    ...
    const expected = await this.ctx.storage.get<{ runtimeGeneration: string }>("execution") ?? await execution(this.driver.getSession().identity.sessionId);
    const pin = (command.value as { execution: { machineId: string; runtimeGeneration: string } }).execution;
    const input = parseApplyPatchInput(this.driver.getSession().config);
    if (pin.machineId !== input.machineId || pin.runtimeGeneration !== expected.runtimeGeneration) throw Error("Completion does not match the session execution pin.");
...
async function execution(sessionId: string) {
  ...
  return {...stored, token: await issueMachineSecret("apply-patch-test-signing-secret-at-least-32-bytes", "00000000-0000-4000-8000-000000000001", "execution", 1)};
}
