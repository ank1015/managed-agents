...
import type { ApplyPatchInput, ApplyPatchSubmission } from "@managed-agents/contracts";
...
export class Gateway {
  ...
  async submit(input: ApplyPatchInput, parsed: ApplyPatchSubmission, requestId: string, context: ApplyPatchContext): Promise<void> {
    ...
    if (new TextEncoder().encode(encoded).byteLength > MAX_HTTP_BYTES) throw new GatewayError(413, "APPLY_PATCH_REQUEST_TOO_LARGE", "Patch request exceeds the gateway transfer limit.");
...
export function patchParams(input: ApplyPatchInput) {
  return { cwd: input.cwd, patch: { format: "codex", text: input.patch } };
}
