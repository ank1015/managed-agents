...
import { FakeGateway as BashGateway, event as bashEvent } from "../../tool-pi-bash-workers/test/stack.ts";
import { FakeGateway as ReadGateway, FakeImages, imageAccountId, fileResult, png, event as readEvent } from "../../tool-pi-read-workers/test/stack.ts";
import { FakeGateway as WriteGateway, event as writeEvent } from "../../tool-pi-write-workers/test/stack.ts";
import { FakeGateway as EditGateway, event as editEvent, signed as signExecution } from "../../tool-pi-edit-workers/test/stack.ts";
...
for (const provider of ["openai", "fireworks"] as const) test(`${provider}: production worker stack validates v5 callbacks, uploads image, and replays all four tool results`, async () => {
  const names = ["bash", "read", "edit", "write"] as const;
  const [api, host, llm, callbacks, ...scripts] = await Promise.all([
    bundle("../../agent-api/src/index.ts"), bundle("../src/index.ts"), bundle("../../llm-gateway-workers/src/index.ts"),
    bundle("../../execution-gateway-callback-workers/src/index.ts"), ...names.map(n => bundle(`../../tool-pi-${n}-workers/src/index.ts`)),
  ]);
  ...
  const app = new Miniflare({ log: new Log(LogLevel.ERROR), workers: [
    ...
    { ...common, name: "host", script: host, d1Databases: { SESSION_DIRECTORY: "directory" },
      ...
      serviceBindings: { LLM: { name: "llm", entrypoint: "LlmGateway" },
        ...Object.fromEntries(names.map(n => [n.toUpperCase(), { name: n, entrypoint: `Pi${n[0]!.toUpperCase()}${n.slice(1)}` }])) }, outboundService: denyNetwork },
    ...
    { ...common, name: "callbacks", script: callbacks,
      bindings: { EXECUTION_GATEWAY_WEBHOOK_SECRET: "test-webhook-secret", CALLBACK_ROUTES: JSON.stringify(Object.fromEntries(names.map(n => [`tool-pi-${n}-v1`, `${n.toUpperCase()}_EVENTS`]))) },
      serviceBindings: Object.fromEntries(names.map(n => [`${n.toUpperCase()}_EVENTS`, { name: n, entrypoint: `Pi${n[0]!.toUpperCase()}${n.slice(1)}Callbacks` }])), outboundService: denyNetwork },
  ] });
  try {
    ...
    const created = await (await request("/v1/sessions", { requestId: "real-workers", harness: { id: "pi-no-compaction", version: "v1" }, config: { ...config, provider, modelId }, metadata: {} })).json() as { session: { identity: { sessionId: string } } };
    ...
    async function deliver(event: unknown) {
      const response = await callbackWorker.fetch("https://callbacks/webhooks/execution-gateway", signExecution(event));
      assert.equal(response.status, 204, await response.text());
    }
    const w = [...writeGateway.jobs.values()][0]!; writeGateway.finish(w); await deliver(writeEvent(w));
    await until(async () => editGateway.jobs.size, size => size === 1);
    const e = [...editGateway.jobs.values()][0]!; editGateway.finish(e); await deliver(editEvent(e));
    const b = [...bashGateway.jobs.values()][0]!; bashGateway.finish(b); await deliver(bashEvent(b));
    await until(page, p => p.state.activeToolCount === 1);
    assert.equal(llmGateway.jobs.size, 1);
    const r = [...readGateway.jobs.values()][0]!;
    readGateway.finish(r, { ...fileResult(png), path: "/workspace/image.png" });
    const readCallback = readEvent(r); await deliver(readCallback); await deliver(readCallback);
