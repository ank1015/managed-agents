...
export type Page = { messages: { sequence: number; message: { role: string; tag?: string; content?: unknown[] }; inContext: boolean }[]; nextCursor: number | null;
  state: { status: string; phase: string; runId: string; pendingMessageCount: number; activeOperationId: string | null; activeToolCount: number; processingBlocked: boolean; error: JsonValue | null } };
...
export async function startStack(persistPath?: string) {
  ...
  const ns = { PI_NO_COMPACTION_SESSIONS: { className: "PiNoCompactionSessionV1", scriptName: "host" } };
  ...
  const app = new Miniflare({ log: new Log(LogLevel.ERROR), workers: [
    ...
    { ...common, name: "host", script: host, d1Databases, bindings: { }, outboundService: { name: "operations" }, durableObjects: { PI_NO_COMPACTION_SESSIONS: { className: "PiNoCompactionSessionV1", useSQLite: true } },
    ...
    return { app, db, request,
      async create(key: string, overrides = {}) {
        const result = await request<{ session: { identity: { sessionId: string } } }>("/v1/sessions", "POST", {
          requestId: key, harness: { id: "pi-no-compaction", version: "v1" }, config: { ...config, ...overrides }, metadata: { title: key },
      ...
      input: (id: string, eventId: string, type = "pi_no_compaction.message", payload: unknown = { message: { role: "user", content: [{ type: "text", text: eventId }] } }) =>
