...
import { parseBashSubmission, parseReadSubmission, parseEditSubmission, parseWriteSubmission, parseLlmSubmission, parseProviderStatusQuery } from "@managed-agents/contracts";
...
export class FakeJobs extends DurableObject<TestEnv> {
  ...
  async submit(serialized: string): Promise<string> {
    ...
    const parser = { llm: parseLlmSubmission, "tool-pi-bash": parseBashSubmission, "tool-pi-read": parseReadSubmission,
      "tool-pi-edit": parseEditSubmission, "tool-pi-write": parseWriteSubmission }[raw.submission.request.provider];
    if (!parser) throw new Error("Unsupported provider");
    const request = parser(raw);
  ...
  async finish(id: string, serializedOutcome: string, deliver = true): Promise<string> {
    ...
    const ns = this.env.PI_NO_COMPACTION_SESSIONS;
...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    if (path === "/internal") {
      ...
      return Response.json(await env.PI_NO_COMPACTION_SESSIONS.get(env.PI_NO_COMPACTION_SESSIONS.idFromName(body.sessionId)).sessionRequest(body.command));
    }
