...
import { PI_READ_OPERATION, parseReadInput, parseJsonValue, parseProviderSubmitReply, parseSessionCommand, parseGatewayEventReply } from "@managed-agents/contracts";
import type { EventBody, JsonValue, ReadWorkerBinding, GatewayEventReceiverBinding, SessionReply } from "@managed-agents/contracts";
...
interface TestEnv extends Env { READ: ReadWorkerBinding; READ_EVENTS: GatewayEventReceiverBinding; SESSIONS: DurableObjectNamespace<TestSession> }
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (path === "/event") return Response.json(parseGatewayEventReply(await env.READ_EVENTS.acceptGatewayEvent(body)));
