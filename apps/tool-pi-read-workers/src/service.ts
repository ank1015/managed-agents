...
import {
  ContractException, PI_READ_OPERATION, PI_READ_RECEIVER, parseReadInput, parseReadSubmission,
...
export class ReadService {
  ...
  submit(value: unknown): Promise<ProviderSubmitResult> {
    const logger = new Logger("read-worker", this.env), started = Date.now();
    ...
    return withDeadline<ProviderSubmitResult>(async signal => {
      ...
      try {
        ...
        if (request.provider !== PI_READ_OPERATION.provider || request.type !== PI_READ_OPERATION.type || request.version !== PI_READ_OPERATION.version) {
          return { status: "rejected", error: { code: "UNSUPPORTED_PI_READ_OPERATION", message: "Expected tool-pi-read/read/v1." } };
        }
      ...
      const clientContext = { receiver: PI_READ_RECEIVER, ...parsed.destination, operationId, submissionId,
        machineId: input.machineId, path: input.path, offset: input.offset ?? null, limit: input.limit ?? null };
  ...
  acceptGatewayEvent(value: unknown): Promise<GatewayEventReceipt> {
    ...
    return withDeadline(async signal => {
      const event = parseExecutionGatewayEvent(value), context = parseReadContext(event.clientContext);
      if (event.machineId !== context.machineId || event.idempotencyKey !== await gatewayKey(context.submissionId)) {
        throw new Error("Gateway event conflicts with read submission context.");
      }
      ...
      const outcome = await terminalOutcome({ id: event.jobId, status: event.type.slice(4) as JobStatus,
        ...
        event.response, event.error, context, this.env, signal);
      signal.throwIfAborted();
      const reply = await namespace.get(namespace.idFromName(context.sessionId)).sessionRequest({
        action: "acceptCompletion", value: { provider: "tool-pi-read", operationId: context.operationId,
      ...
      if (!(reply as { value: { duplicate: boolean } }).value.duplicate) {
        const logger = new Logger("read-worker", this.env);
        const fields = {
...
function gatewayKey(submissionId: string): Promise<string> { return sha256(submissionId).then(hash => `pi-read-v1:${hash}`); }
