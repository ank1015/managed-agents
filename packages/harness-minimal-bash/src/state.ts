...
import type { JsonValue, LlmMessage, SessionStatus } from "@managed-agents/contracts";
import { deleteJson, readJson, readJsonText, storedJsonBytes, writeJson } from "@managed-agents/sqlite-json";

export type Sql = Pick<SqlStorage, "exec">;
const CHUNKS = "minimal_bash_json_chunks";
export const storeMessageJson = (sql: Sql, value: unknown) => writeJson(sql, CHUNKS, value);
export const readMessageJson = <T>(sql: Sql, stored: string) => readJson<T>(sql, CHUNKS, stored);
export const readMessageText = (sql: Sql, stored: string) => readJsonText(sql, CHUNKS, stored);
export const deleteMessageJson = (sql: Sql, stored: string) => deleteJson(sql, CHUNKS, stored);
export type State = {
...
export function sessionStatus(state: State): SessionStatus {
  if (state.cancel_requested && (state.phase === "llm" || state.phase === "bash")) return "cancelling";
  return state.phase === "llm" || state.phase === "bash" ? "running" : state.phase;
}
export function readMinimalBashState(sql: Sql) {
  const s = readState(sql);
  return { status: sessionStatus(s), phase: s.phase, runId: s.run_id, activeOperationId: s.active_operation_id,
...
export function readMinimalBashMessages(sql: Sql, after = 0, limit = 100) {
  const page = pageRows(sql.exec<{ sequence: number; message_json: string; in_context: number; run_id: string | null; response_metadata_json: string | null }>(
    ...
    limit, row => storedJsonBytes(row.message_json) + storedJsonBytes(row.response_metadata_json ?? "null"));
  const messages = page.rows.map(row => ({ sequence: row.sequence, message: readMessageJson<LlmMessage>(sql, row.message_json),
    inContext: Boolean(row.in_context), runId: row.run_id,
    responseMetadata: row.response_metadata_json === null ? null : readMessageJson<JsonValue>(sql, row.response_metadata_json) }));
...
export function readPendingMessages(sql: Sql, after = 0, limit = 100) {
  const page = pageRows(sql.exec<{ event_id: string; input_sequence: number; message_json: string }>(
    ...
    limit, row => storedJsonBytes(row.message_json));
  const messages = page.rows.map(row => ({ eventId: row.event_id, inputSequence: row.input_sequence, message: readMessageJson<LlmMessage>(sql, row.message_json) }));
...
export function appendMessage(sql: Sql, message: LlmMessage, runId: string | null, options: {
  ...
  return sql.exec<{ sequence: number }>(`INSERT INTO minimal_bash_messages
    ...
    VALUES (?, ?, ?, ?, ?, ?) RETURNING sequence`, storeMessageJson(sql, message), options.inContext === false ? 0 : 1,
    runId, options.eventId ?? null, options.operationId ?? null,
    options.responseMetadata === undefined ? null : storeMessageJson(sql, options.responseMetadata)).one().sequence;
}
