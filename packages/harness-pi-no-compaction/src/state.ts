...
export type State = {
  singleton: number; phase: "idle" | "llm" | "tools" | "cancelled" | "failed";
...
export function readState(sql: Sql): State { return sql.exec<State>("SELECT * FROM pi_no_compaction_state WHERE singleton = 1").one(); }
export function readPiNoCompactionState(sql: Sql) {
  ...
  return { phase: s.phase, runId: s.run_id, activeOperationId: s.active_operation_id,
    ...
    activeToolCount: sql.exec<{ n: number }>("SELECT COUNT(*) AS n FROM pi_no_compaction_batch WHERE status = 'active'").one().n,
    pendingMessageCount: sql.exec<{ n: number }>("SELECT COUNT(*) AS n FROM pi_no_compaction_pending_messages").one().n };
}
...
export function readPiNoCompactionMessages(sql: Sql, after = 0, limit = 100) {
  const page = pageRows(sql.exec<{ sequence: number; message_json: string; in_context: number; run_id: string | null; response_metadata_json: string | null }>(
    "SELECT sequence, message_json, in_context, run_id, response_metadata_json FROM pi_no_compaction_messages WHERE sequence > ? ORDER BY sequence LIMIT ?", after, limit + 1),
  ...
  return { messages, nextCursor: page.hasMore ? messages.at(-1)!.sequence : null, state: readPiNoCompactionState(sql) };
}
export function readPendingMessages(sql: Sql, after = 0, limit = 100) {
  const page = pageRows(sql.exec<{ event_id: string; input_sequence: number; message_json: string }>(
    "SELECT * FROM pi_no_compaction_pending_messages WHERE input_sequence > ? ORDER BY input_sequence LIMIT ?", after, limit + 1),
  ...
  return { messages, nextCursor: page.hasMore ? messages.at(-1)!.inputSequence : null, state: readPiNoCompactionState(sql) };
}
export function appendMessage(sql: Sql, message: LlmMessage, runId: string | null, options: {
  ...
  return sql.exec<{ sequence: number }>(`INSERT INTO pi_no_compaction_messages
...
export type BatchRow = { call_index: number; status: "queued" | "active" | "done"; operation_id: string | null; result_json: string | null };
export function readBatch(sql: Sql): BatchRow[] { return sql.exec<BatchRow>("SELECT * FROM pi_no_compaction_batch ORDER BY call_index").toArray(); }
