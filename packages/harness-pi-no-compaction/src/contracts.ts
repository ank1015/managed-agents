import { ContractException, isAbsoluteMachinePath, parseEventBody, parseJsonValue, parseLlmMessage, parseUuid, utf8Bytes } from "@managed-agents/contracts";
...
type CommonConfig = { accountId: string; reasoning: ReasoningLevel; machineId: string; cwd: string; maxOutputTokens: number };
...
export function parsePiNoCompactionConfig(value: JsonValue): PiNoCompactionConfig {
  try {
    const c = fields(value, ["provider", "modelId", "accountId", "reasoning", "machineId", "cwd", "maxOutputTokens"]);
    ...
    return { provider: c.provider, modelId: c.modelId, accountId: parseUuid(c.accountId), reasoning: reasoning as ReasoningLevel,
      machineId: parseUuid(c.machineId), cwd: c.cwd, maxOutputTokens } as PiNoCompactionConfig;
