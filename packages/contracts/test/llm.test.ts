...
import { LLM_OPERATION, parseLlmInput, parseLlmResponse, parseOperationRequest, parseOperationOutcome, MAX_OPERATION_INPUT_BYTES, MAX_OPERATION_OUTCOME_BYTES } from "../src/index.ts";
...
test("8 MiB requests and 1.9 MB outcomes count UTF-8 bytes including JSON overhead", () => {
  assert.equal(MAX_OPERATION_INPUT_BYTES, 8 * 1024 * 1024);
  assert.equal(MAX_OPERATION_OUTCOME_BYTES, 1_900_000);
  ...
  outcome.result = "x".repeat(MAX_OPERATION_OUTCOME_BYTES - JSON.stringify(outcome).length);
  assert.deepEqual(parseOperationOutcome(outcome), outcome);
  assert.throws(() => parseOperationOutcome({ ...outcome, result: outcome.result + "x" }), /exceeds/);
  assert.throws(() => parseOperationOutcome({ ...outcome, result: "😀".repeat(MAX_OPERATION_OUTCOME_BYTES / 4) }), /exceeds/);
