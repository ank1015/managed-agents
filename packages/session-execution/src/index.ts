...
import { ContractException, parseExecutionGatewayUrl, parseProviderSubmitReply } from "@managed-agents/contracts";
...
interface RuntimeBinding { gatewayUrl: string; machineId: string; runtimeGeneration: string; destination: SessionDestination }
interface ExecutionConfig { executionGatewayUrl: string; machineId: string; executionToken: string }
...
export class SessionExecution {
  ...
  private async http(gatewayUrl: string, path: string, token: string): Promise<Record<string, unknown>> {
    const response = await fetch(`${gatewayUrl}${path}`, { method: "GET", redirect: "manual",
  ...
  private async prepare(destination: SessionDestination, gatewayUrl: string, machineId: string, token: string): Promise<void> {
    if (this.runtime()) return;
    const response = await this.http(gatewayUrl, `/v1/machines/${machineId}`, token), machine = object(response.machine);
    if (machine.machineId !== machineId || machine.connectionStatus !== "ready") throw Error("Execution machine is not ready.");
    const runtime: RuntimeBinding = { gatewayUrl, machineId, runtimeGeneration: uuid(machine.runtimeGeneration), destination };
  ...
  async submit(binding: { submit(value: unknown): Promise<unknown> }, destination: SessionDestination, submission: ProviderSubmission, config: ExecutionConfig): Promise<ProviderSubmitResult> {
    ...
    const gatewayUrl = parseExecutionGatewayUrl(config.executionGatewayUrl);
    ...
    if (!this.runtime()) {
      this.preparing ??= this.prepare(destination, gatewayUrl, machineId, config.executionToken).finally(() => { this.preparing = undefined; });
      await this.preparing;
    }
    const runtime = this.runtime()!;
    if (runtime.gatewayUrl !== gatewayUrl || runtime.machineId !== machineId || runtime.destination.sessionId !== destination.sessionId || runtime.destination.routeKey !== destination.routeKey) throw Error("Execution runtime identity mismatch.");
    return parseProviderSubmitReply(await binding.submit({ destination, submission, execution: {
      gatewayUrl, token: config.executionToken, runtimeGeneration: runtime.runtimeGeneration,
    } }));
  }
  ...
  toolCompletion(value: unknown): unknown {
    ...
    try {
      ...
      execution = object(raw.execution, ["gatewayUrl", "machineId", "runtimeGeneration"]);
    ...
    if (execution.gatewayUrl !== runtime.gatewayUrl || execution.machineId !== runtime.machineId
